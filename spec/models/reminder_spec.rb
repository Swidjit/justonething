...
describe Reminder do
  ...
  describe "sends for single events" do
    before(:each) {
      time = Date.today.to_time + 1.day + 12.hours
    ...
    it 'should send reminder email' do
      ...
      Reminder.send_for_date (1.day.from_now)
  ...
  describe "sends for recurring events" do
    before(:each) {
      ...
      @event.save
      @user = Factory :user
      @reminder = Factory :reminder, item: @event, user: @user, date: @event.next_occurrence(2.days.from_now).to_date
    }

    # Passes individually, fails when running entire suite
    # it 'should send reminder email' do
