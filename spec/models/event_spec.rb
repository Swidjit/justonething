...
describe Event do
  ...
  describe "#event_recurrences" do
    ...
    it "should be daily" do
      @event.rules = {0=>{rule: 'daily'}}
      @event.save
      @event.reload.rules.first.is_daily?.should == true
    end
    it "should be weekly" do
      @event.rules = {0=>{rule: 'weekly', weekly_day: 3}}
      @event.save
      @event.reload.rules.first.is_weekly?.should == true
    end
    it "should be monthly by week/day" do
      @event.rules = {0=>{rule: 'monthly_week', monthly_week: 1, monthly_day: 2}}
      @event.save
      @event.reload.rules.first.is_monthly_week?.should == true
    end

    it "should be monthly by day of month" do
      @event.rules = {0=>{rule: 'monthly_date', monthly_date: 15}}
      @event.save
      @event.reload.rules.first.is_monthly_date?.should == true
      @event.rules.first.monthly_date.should == 15
    end
    ...
    it "should update rule expiry when updating event expiry" do
      ...
      @event.expires_on = 1.year.from_now
      @event.save
      @event.schedule.end_time.to_time.utc.to_s.should == 1.year.from_now.to_time.to_s
      #@event.reload.rules.first.ice_rule.until_time.to_time.to_s.should == 1.year.from_now.to_time.to_s
    end

    # should also test removal of rules through "no rules"
  end
